...
function App() {
  const [state, setState] = useState<EthereumState>({
    signer:null , contract:null , provider:null , account:null
  });
  ...
  const getData = async () => {
    const updatedName: string = await state.contract?.getName();
  ...
  const handleSubmit = async(e:React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (state.contract) {
      await state.contract?.setName(data);
  ...
  return (
    <div>
      <form action="" onSubmit={handleSubmit}>
        <input onChange={(e:React.ChangeEvent<HTMLInputElement>)=>setData(e.target.value)} className="inputType" type="text" placeholder="Plz enter name"/>
